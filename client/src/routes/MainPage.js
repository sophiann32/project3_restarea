...
function MainPage() {
    ...
    const [isPlaceholderVisible, setPlaceholderVisible] = useState(true); // 깜빡임 상태 추가

    const intervalRef = useRef(null);
    const blinkIntervalRef = useRef(null); // 깜빡임 간격을 위한 ref 추가
    ...
    useEffect(() => {
        ...
        intervalRef.current = setInterval(addChar, 130);

        return () => {
            clearInterval(intervalRef.current);
            clearInterval(blinkIntervalRef.current); // cleanup 깜빡임 간격
        };
                    ...
                    <li>
                        ...
                        >
                            <option value="" disabled hidden>
                                {placeholderText ? (isPlaceholderVisible ? placeholderText : "") : "로딩 중..."} {/* 깜빡임 효과 적용 */}
                            </option>
